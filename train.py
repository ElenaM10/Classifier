...
def test(device, all_loaders, model, criterion, optimizer):
    epochs = 1
    steps = 0
    running_loss = 0
    print_every =20
...
def saving_checkpoint(arch, model,save_dir, input_size, epochs, all_datasets, optimizer):
    ...
    #torch.load(os.path.join(in_args.save_dir, "checkpoint.pth"), map_location=lambda storage, loc: storage)
    torch.save(save_dir , '/home/workspace/ImageClassifier/vgg16/checkpoint.pth')
...
def main():
    ...
    test(device, all_loaders, model, criterion, optimizer)
    saving_checkpoint(arch, model,save_dir, input_size, epochs, all_datasets, optimizer)
